...
class LR_LightGCN(nn.Module):
    def __init__(self, args, dataset, interaction_matrix):
        super(LR_LightGCN, self).__init__()
    ...
    def get_ego_embeddings(self):
        ...
        resd_emb=torch.matmul(user_embeddings, item_embeddings.transpose(0, 1))
        return ego_embeddings,resd_emb                              ###第0层的嵌入和残差

    def forward(self):
        all_embeddings,resd_emb = self.get_ego_embeddings()
        embeddings_list = [all_embeddings]
        for layer_idx in range(self.n_layers):
            all_embeddings = torch.sparse.mm(self.norm_adj_matrix, all_embeddings)          ###卷积过程
            embeddings_list.append(all_embeddings)                                          ###存入每层的卷积结果
            resgcn_all_embeddings = torch.stack(embeddings_list, dim=1)
            resgcn_all_embeddings = torch.mean(resgcn_all_embeddings, dim=1)
            user_res_embeddings, item_res_embeddings = torch.split(resgcn_all_embeddings,
                                                                   [self.n_users, self.n_items])
            resd_emb+= torch.matmul(user_res_embeddings, item_res_embeddings.transpose(0, 1))
        ...
        return user_all_embeddings, item_all_embeddings,resd_emb
    ...
    def calculate_loss(self, interaction):
        ...
        user_all_embeddings, item_all_embeddings,resd_emb = self.forward()
        ...
        pos_scores=resd_emb[user][pos_item].sum(dim=1)
        neg_scores =resd_emb[user][neg_item].sum(dim=1)

...
class Aggregator(object):
    ...
    def _mix_neighbor_vectors(self, neighbor_vectors, neighbor_relations, user_embeddings):
        ...
        if not avg:
            ...
            user_relation_scores_normalized = tf.compat.v1.nn.softmax(user_relation_scores, dim=-1)
...
class BiAggregator(Aggregator):
    def __init__(self, batch_size, dim, dropout=0., act=tf.nn.leaky_relu, name=None):
    ...
    def _call(self, self_vectors, neighbor_vectors, neighbor_relations, user_embeddings):
        ...
        output1 = tf.reshape(tf.multiply(self_vectors, neighbors_agg), [-1, self.dim])
